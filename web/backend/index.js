...
// 2. ดึง stats รายเหรียญ (รองรับ 'latest' = เหรียญล่าสุดของแต่ละตัว)
app.get('/api/runs/:id/stats/:symbol', (req, res) => {
  const { id, symbol } = req.params;
  const ddb = openDb();

  // ถ้า id = 'latest' → ดึง run_timestamp ล่าสุดของ symbol นี้โดยตรง
  // ถ้า id = 'latest' ให้ดึง trades ล่าสุดของ symbol นี้ โดยไม่สน run_id
  if (id === 'latest') {
    ddb.get(
      `SELECT 
          MAX(t.open_time) as run_at,
          r.config_hash,
          r.max_drawdown_pct as run_max_drawdown_pct,
      ...
      (err, row) => {
        ...
        res.json({
          ...
          stats: {
            ...
            max_drawdown_pct: row.run_max_drawdown_pct || 0,
            win_rate: wr,
    ...
    // ดึงข้อมูลตาม run_timestamp เดิม
    ddb.get(
      `SELECT 
...
// 3. ดึง Trades แบบ Pagination + Sorting (รองรับ 'latest')
app.get('/api/runs/:id/trades/:symbol', (req, res) => {
  ...
  // ถ้า id = 'latest' → ดึง trades ทั้งหมดของ symbol โดยไม่กรอง run_timestamp
  const countQuery = id === 'latest'
    ? `SELECT COUNT(*) as total FROM trades t WHERE t.symbol = ?`
    : `SELECT COUNT(*) as total FROM trades t INNER JOIN runs r ON t.run_id = r.id WHERE r.run_timestamp = ? AND t.symbol = ?`;

  const countParams = id === 'latest' ? [symbol] : [id, symbol];

  ddb.get(countQuery, countParams, (err, countRow) => {
    if (err) return res.status(500).json({ error: err.message });
    
    const total = countRow?.total || 0;
    const offset = (page - 1) * limit;

    const dataQuery = id === 'latest'
      ? `SELECT t.* FROM trades t WHERE t.symbol = ? ORDER BY ${sortCol} ${order} LIMIT ? OFFSET ?`
      : `SELECT t.* FROM trades t INNER JOIN runs r ON t.run_id = r.id WHERE r.run_timestamp = ? AND t.symbol = ? ORDER BY ${sortCol} ${order} LIMIT ? OFFSET ?`;

    const dataParams = id === 'latest' ? [symbol, limit, offset] : [id, symbol, limit, offset];
    
    ddb.all(dataQuery, dataParams, (err2, rows) => {
  ...
  if (id === 'latest') {
    ...
    openDb().get(
      `SELECT 
        ...
        COUNT(*) as total_trades,
        MAX(t.dca_count) as max_dca_any_symbol
       FROM trades t`,
      (err, row) => {
        if (err || !row) return res.status(404).json({ error: 'No data' });
        res.json({
          symbol: 'ALL',
          stats: { 
            total_pnl_usd: row.total_pnl_usd || 0, 
          ...
          trades_summary: { 
            max_dca_count: row.max_dca_any_symbol, 
            liquidations: 0 
          }
        });
      }
