...
def run_backtest(symbol: str, cfg: dict = None) -> dict:
    ...
    # ======== STEP 1: Compute ALL signals (vectorized) ========
    ...
    # ======== STEP 2: Simulate positions (sequential, only signal candles matter) ========
    ...
    # For tracking
    current_order = None  # {"type": "LONG"/"SHORT", "price": float, "active": bool}
    ...
    for i in range(1, total_rows):
        ...
        if position is not None and position.status == "OPEN":
            ...
            # 1) Check TP
            side = position.side
            if (side == "LONG" and price >= position.bep * (1 + tp_pct)) or \
               (side == "SHORT" and price <= position.bep * (1 - tp_pct)):
            ...
            # 2) Check Timeout
            if (side == "LONG" and position.holding_time_minutes >= long_timeout) or \
               (side == "SHORT" and position.holding_time_minutes >= short_timeout):
                # รีเฟรช order (ยกเลิกเก่า วางใหม่ที่ BEP + 0.2%)
                if current_order:
                    current_order = None
                # วาง order ใหม่
                target_price = position.take_profit_price
                current_order = {"type": side, "price": target_price, "active": True}
                portfolio.update_equity(ts)
                progress.update(1)
                continue

            # 3) Check DCA
            if (side == "LONG" and price <= position.bep * (1 - dca_pct)) or \
               (side == "SHORT" and price >= position.bep * (1 + dca_pct)):
                if position.total_size_usd + size_per_trade <= max_cap:
                    # เปิด DCA
            ...
            if current_order and current_order["active"]:
                ...
                if (side == "LONG" and price >= order_price) or \
                   (side == "SHORT" and price <= order_price):
                    # TP order matches!
                    position.close(ts, price, fee_rate, "TP")
                    pnl = position.pnl(price)
                    portfolio.record_trade(TradeLog(
        ...
        # Cooldown: ถ้าเพิ่งปิด position ไป อย่าเพิ่งเปิดใหม่ทันที (รอ 5 นาที)
        # (แค่นี้ทำให้ stable ขึ้น)
        ...
        if signal_long[i]:
            ...
            current_order = {"type": "LONG", "price": pos.take_profit_price, "active": True}
            position = pos
            portfolio.update_equity(ts)
            ...
            pos.add_trade(ts, "OPEN", order_price, size_per_trade, fee_rate)
            current_order = {"type": "SHORT", "price": pos.take_profit_price, "active": True}
            position = pos
            portfolio.update_equity(ts)
