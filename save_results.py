...
def save_one(symbol, cfg, results_dir="results"):
    ...
    result = {
        ...
        "stats": {
            ...
            "total_volume_usd": round(trades['size_usd'].sum(), 2),
        },
        "trades_summary": {
            "max_dca_count": int(trades['dca_count'].max()),
            "avg_dca_count": round(trades['dca_count'].mean(), 2),
            "median_dca_count": int(trades['dca_count'].median()),
            "max_holding_minutes": int(trades['holding_minutes'].max()),
            "avg_holding_minutes": round(trades['holding_minutes'].mean(), 1),
        ...
        "margin_analysis": {
            "max_margin_used_usd": round((int(trades['dca_count'].max()) + 1) * 200 / 10, 2),
            "free_margin_remaining": round(50000 - (int(trades['dca_count'].max()) + 1) * 200 / 10, 2),
        },
        "side_breakdown": {
            "long": {
                "count": int((trades['side'] == 'LONG').sum()),
                "pnl": round(float(trades[trades['side'] == 'LONG']['pnl_usd'].sum()), 2),
                "max_dca": int(trades[trades['side'] == 'LONG']['dca_count'].max()) if len(trades[trades['side'] == 'LONG']) > 0 else 0,
            },
            "short": {
                "count": int((trades['side'] == 'SHORT').sum()),
                "pnl": round(float(trades[trades['side'] == 'SHORT']['pnl_usd'].sum()), 2),
